...
namespace Resource
{
	Window::Window(UINT width, UINT height)
	{
		...
		ASSERT_HR(dxgi_factory->CreateSwapChain(Platform::GPU::Device().Get(), &backBufferDesc, m_swapChain.GetAddressOf()));

		m_swapChain->GetBuffer(0, IID_PPV_ARGS(m_backBuffer.GetAddressOf()));
		ASSERT_HR(Platform::GPU::Device()->CreateRenderTargetView(m_backBuffer.Get(), NULL, m_backBufferRTV.GetAddressOf()));
		...
		ASSERT_HR(Platform::GPU::Device()->CreateTexture2D(&depthBufferDesc, NULL, m_depthBuffer.GetAddressOf()));
		...
		ASSERT_HR(Platform::GPU::Device()->CreateDepthStencilView(m_depthBuffer.Get(), &depthStencilViewDesc, m_depthBufferDSV.GetAddressOf()));
	}
	...
	void Window::Clear(float red, float green, float blue, float alpha)
	{
		...
		Platform::GPU::Context()->ClearRenderTargetView(m_backBufferRTV.Get(), clearColor);
		Platform::GPU::Context()->ClearDepthStencilView(m_depthBufferDSV.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	}
	...
	void Window::Bind(float top, float left, float bottom, float right)
	{
		...
		Platform::GPU::Context()->RSSetViewports(1, &viewPort);
		Platform::GPU::Context()->OMSetRenderTargets(1, m_backBufferRTV.GetAddressOf(), m_depthBufferDSV.Get());
	}
